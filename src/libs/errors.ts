...
export class ApiError extends Error {
  ...
  static cause(error: Error): ApiError {
    if (error instanceof AxiosError) {
      if (error.response) {
        return ApiError.resolve(error.response.data.code, error.response.data.message);
      }
      return ApiError.resolve(Number.parseInt(error.code ?? '500', 10), error.message);
    }
    return ApiError.resolve(500, error.message);
  }
